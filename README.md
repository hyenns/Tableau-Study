...
이 레포지토리에는 태블로로 만든 다양한 시각화 그래프 예시들이 포함되어 있습니다. ✨
...
# 26. ChatGPT 출시 이후 직무별 일감 변화 그래프
![ChatGPT 출시 이후 직무별 일감 변화 그래프](ChatGPT_출시_이후_일감_수_변화.png)

이 그래프는 ChatGPT 출시 이후 직무별 일감 변화 비율을 나타낸 수평 막대 그래프입니다.

주요 요소 설명:
- 세로축:다양한 직무 카테고리를 나열하고 있습니다. 각 카테고리는 직무의 종류를 나타냅니다.
- 가로축: % 변화율을 나타내며, 각 직무에서 ChatGPT의 영향을 받아 증가하거나 감소한 비율을 보여줍니다. 변화율은 -30%에서 35%까지의 범위를 가집니다.
- 막대: 막대의 길이는 해당 직무에서의 변화 비율을 나타냅니다. 막대가 왼쪽으로 이동하면 해당 직무의 일감이 감소했음을 의미하며, 오른쪽으로 이동하면 증가했음을 의미합니다.

주요 관찰 포인트:

- 일감 감소 직무: Writing(작문), Web Development(웹 개발), Social Media Marketing(소셜 미디어 마케팅) 등의 직무에서 일감이 감소한 것으로 나타납니다. 특히 작문 직무는 -30%에 가까운 감소율을 보입니다.
- 일감 증가 직무: Market Research(시장 조사)와 같은 직무는 변화율이 10% 이상으로 증가한 것으로 보이며, 이는 ChatGPT의 도입으로 해당 분야의 수요가 증가했음을 나타냅니다.
- 중립적 변화: Sales(영업)와 Translation(번역) 등의 직무는 변화율이 상대적으로 중립적이거나 소폭 증가한 경향을 보입니다.

이 그래프는 ChatGPT의 도입이 다양한 직무에 미친 영향을 분석하여, 특정 직무가 어떻게 변화했는지를 파악하는 데 유용한 정보를 제공합니다. 이를 통해 기업이나 개인이 향후 전략을 세울 때 참고할 수 있는 인사이트를 제공할 수 있습니다.
